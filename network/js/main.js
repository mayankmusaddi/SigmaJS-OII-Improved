...
function configSigmaElements(config) {
    ...
    if (config.features.hoverBehavior == "dim") {
        ...
        //nodes stores the current hovered node
    ...
    this.search = function (a) {
        ...
        for(var i=0;i<arr.length;i++)
        {
            ...
            else
            {
                ...
                if(found == !1) sr.push("<i>"+this.lastSearch+": No results found.</i>");
                else sr.push('<a href="#' + c[clen].name + '" onclick="nodeActive(\'' + c[clen].id + "')\">" + c[clen].name + "</a>");
            }
        }
...
function nodeNormal() {
    !0 != $GP.calculating && !1 != sigInst.detail && (showGroups(!1), $GP.calculating = !0, sigInst.detail = !0, $GP.info.delay(400).animate({width:'hide'},350),$GP.cluster.hide(), sigInst.iterEdges(function (a) {
        ...
        a.color = a.attr['selected'] ? a.attr['select_color'] : a.color;
        a.attr['selected'] = 0;
...
function nodesActive(c) {
    ...
    for (var d = 0, h = c.length; d < h; d++)
    {
        ...
        sigInst.iterEdges(function (b) {
            ...
            if (a == b.source || a == b.target) sigInst.neighbors[a == b.target ? b.source : b.target] = n; // adding to sigInst.neighbours displays all the neighbours
